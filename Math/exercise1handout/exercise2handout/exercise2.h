...
struct Exercise2 {
	...
	void update() {
		...
		//para rotar la camara con la posición del ratón utilizamos yaw para rotar en el eje Y y pitch para el eje x, por lo que los multiplicamos por el valor correspondiente en quat_from_axis_deg
		...
		//para rotar el diorama utilizamos hacemos como en la rotación de la cámara.
		//Utilizando su ángulo Yaw, lo incrementamos gradualmente y lo aplicamos a su eje Y, generando así una rotación constante.

    if (glfwGetKey(window, GLFW_KEY_UP)) {
      cameraPosition.z -= 2.f * elapsed_seconds;
      cameraMatrix = translate(identity_mat4(), cameraPosition * -1.f); //multiplicamos por -1 para invertir la vista de la cámara ya que esta invertida
    }
    ...
    if (glfwGetKey(window, GLFW_KEY_DOWN)) {
      ...
      cameraMatrix = translate(identity_mat4(), cameraPosition * -1.f);
		...
		camera.set_shader_uniforms(mesh_shader_index, camNode.worldInverseMatrix); //Inverso
		...
		camera.set_shader_uniforms(mesh_shader_index, camNode.worldMatrix); //Normal
